...
For users running the GitHub version with Codex or Claude, this repository also includes automatic agent-guided workflows for custom mapper/inverter creation and beta human-in-the-loop feature selection; see [Auto Features](#auto-features).
...
**Auto feature: automatic mapper and inverter creation.** For users who already have selected and preprocessed data, an agent can inspect the data dimensions, update `config/architecture.yaml`, and create matching mapper and inverter neural networks.

Start this in a Codex or Claude agent chat with:

```text
!build_custom_NN
```

If the preprocessed data are missing or saved outside the default preprocessed folder, the agent may ask where the data are saved and update `config/data_path.yaml`.
...
After reviewing and saving the form edited by the agent, create preprocessed data with:
